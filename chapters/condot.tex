...
Such convex functions can be represented using input convex neural networks (ICNN)~\citep{amos2017input}, to parameterize either the Monge map~\citep{jacob2018w2gan,yang2018scalable,bunne2021learning,bunne2022proximal} or a dual potential~\citep{makkuva2020optimal,korotin2020wasserstein} as, respectively, the gradient of an ICNN or an ICNN itself.
...
\looseness -1 \textbf{Contributions }
...
%
Because both ICNN ---and to a greater extent PICNN--- are notoriously difficult to train~\citep{richter2021input,korotin2020wasserstein,korotin2021neural}, we use closed-form solutions between Gaussian approximations to derive relevant parameter initializations for (P)ICNNs:
...
\begin{figure*}
    ...
    \caption{The evolution from a source $\mu$ to a target measure $\nu$ can depend on context variables $c$ of various nature. This comprises \textbf{a.} scalars such as time or dosage $t$ which determine the magnitude of an optimal transport, \textbf{b.} flow of measures into another one based on additional information \newtext{(possibly different between $\mu$ and $\nu$) stored in vectors $\mathbf{c}_{l,l^\prime}$}, or \textbf{c.} discrete and complex actions $a_i$, possibly in combination $a_{ij}$. We seek a unified framework to produce a map $\mathcal{T}_\theta(c)$ from any type of condition $c$. \vspace{-15pt}}
    \label{fig:overview}
\end{figure*}


\section{Supervised Training of Conditional Monge Maps} \label{sec:method}
We are given a dataset of $N$ pairs of measures, each endowed with a label, $(c_i,(\mu_i, \nu_i))\in\mathcal{C}\times \mathcal{P}(\RR^d)^2$. Our framework builds upon two pillars: (i.) we formulate the hypothesis that an optimal transport $T^\star_i$ (or, equivalently, the gradient of a convex potential $f^\star_i$) explains how measure $\mu_i$ was mapped to $\nu_i$, given context $c_i$; (ii.) we build on the multi-task hypothesis \citep{caruana1997multitask} that all of the $N$ maps $T^\star_i$ between $\mu_i$ and $\nu_i$ share a common set of parameters, that are \textit{modulated} by context informations $c_i$. These ideas are summarized in an abstract regression model described below.
...
\subsection{Integrating Context in Convex Architectures}%\paragraph{Towards Parameterized Monge Maps}
...
Using PICNNs as a base module, the \textsc{CondOT} architecture integrates operations on the contexts $\Ccal$. As seen in Figure~\ref{fig:overview}, context values $\cont$ may take various forms:
\begin{enumerate}[noitemsep,leftmargin=.35cm,topsep=0pt,parsep=0pt,partopsep=0pt]
\item A scalar $t$ denoting a strength or a temporal effect. \newtext{For instance,} \citeauthor{mccann1997convexity}'s interpolation and its time parameterization, $\alpha_{t}=((1-t) \text{Id}+t T)_{\sharp} \alpha_{0}$ \citep{mccann1997convexity} can be interpreted as a \newtext{trivial conditional OT model that creates, from an OT map $\psi_\theta$, a set of maps parameterized by $t$}, $\mathcal{T}_\theta(t):=x\mapsto\nabla_x \left((1-t)\|x\|^2/2 +t \psi_\theta(x)\right)$.
...
When the set $\Ccal$ is small, this can be done effectively using one-hot embeddings $\mathcal{E}_\text{ohe}$.
...
% Building on \citeauthor{Brenier1987}'s theorem and following \citet{korotin2020wasserstein, makkuva2020optimal}, we retain the idea that OT maps between two measures $\mu_i$ and $\nu_i$ should be the gradients of a convex function modeled using neural architectures. Unlike previous approaches, however, our goal is to \textit{(i.)} avoid the estimation of a new ICNN for each available pair $(\mu_i,\nu_i)$, and, instead, pool efficiently all samples contained in all pairs of measures to train in a multi-task manner an overall neural architecture with shared weights, while \textit{(ii.)} leveraging context values $c_i$ as much as possible to facilitate joint training and improve generalization.
%
% To reach these two goals, we propose to learn a \emph{unique} parameterized map $\mathcal{T}_\theta$ that outputs, for any context value $\cont$ (either known and seen in the dataset, or unknown), a $\mathbb{R}^d\rightarrow \mathbb{R}^d$ map $\mathcal{T}_\theta(\cont)$ that has the property that if $\cont\approx\cont_i$ then $\mathcal{T}_\theta(\cont)\#\mu_i \approx \nu_i$. This goal can be realized by defining a parameterized potential function $f_\theta:\RR^d\times \Ccal$ that takes that context as an additional input, to define next our parameterized families of OT maps in ~\eqref{eq:picnnmap}. A crucial observation is that the resulting parameterized
% \begin{wrapfigure}[2]{r}{0.34\textwidth}
% \vspace{-.35cm}
% \begin{equation}\label{eq:picnnmap}
% \mathcal{T}_\theta(\cont) := x \mapsto \nabla_1 f_\theta(x,\cont).
% \end{equation}
% \end{wrapfigure}
% potential $\psi_\theta(x,\cont)$ only needs to be input convex in $x$, but not in the context value $\cont$, to ensure that $\mathcal{T}_\theta(\cont)$ is an OT map. 
...
\adjustbox{max width=\linewidth}{%
    ...
    \toprule
         \textbf{Method} & \multicolumn{6}{c}{\textbf{Conditioned on Drug Dosage}} & \multicolumn{2}{c}{\textbf{Conditioned on Cell Line}} \\
         & \multicolumn{2}{c}{In-Sample} && \multicolumn{2}{c}{Out-of-Sample} && \multicolumn{2}{c}{In-Sample} \\
         \cmidrule{2-3} \cmidrule{5-6} \cmidrule{8-9}
         & MMD & $\ell_2(\text{PS})$ && MMD & $\ell_2(\text{PS})$ && MMD & $\ell_2(\text{PS})$ \\
    \midrule
        \textsc{CPA} \citep{lotfollahi2021compositional} & 0.1502 $\pm$ 0.0769 & 2.47 $\pm$ 2.89 && 0.1568 $\pm$ 0.0729 & 2.65 $\pm$ 2.75 && 0.2551 $\pm$ 0.006 & 2.71 $\pm$ 1.51 \\
        \textsc{ICNN OT} \citep{makkuva2020optimal} & 0.0365 $\pm$ 0.0473 & 2.37 $\pm$ 2.15 && 0.0466 $\pm$ 0.0479 & 2.24 $\pm$ 2.39 && 0.0206 $\pm$ 0.0109 & 1.16 $\pm$ 0.75 \\
        \textsc{CondOT} (Identity initialization) & 0.0111 $\pm$ 0.0055 & 0.63 $\pm$ 0.09 && 0.0374 $\pm$ 0.0052 & 2.02 $\pm$ 0.10 && 0.0148 $\pm$ 0.0078 & 0.39 $\pm$ 0.06 \\
        \textsc{CondOT} (Gaussian initialization) & 0.0128 $\pm$ 0.0081 & 0.60 $\pm$ 0.11 && 0.0325 $\pm$ 0.0062 & 1.84 $\pm$ 0.14 && 0.0146 $\pm$ 0.0074 & 0.41 $\pm$ 0.07 \\
    \bottomrule
    \vspace{-20pt}
    \end{tabular}
}
...
% which can provide more generally functions that can predict molecular responses, 
...
\subsection{Population Dynamics Conditioned on \emph{Scalars}} \label{sec:eval_scalar}

% The evolution of single-cell populations during a developmental process can be monitored by sampling, every now and then, representative cells of the system, and measure their gene expression features in order to trace the differentiation across multiple snapshots.
...
% We aim at infering the transport map $T_\theta(\delta t)$ which predicts the development of a cell population at time $t_i$ into cell states at time $t_{i+1}$, i.e., $T_\theta(\delta t)_\# \mu_{i} = \mu_{i+1}$, where $\delta t = t_{i+1} - t_{i}$.
% We learn $T_\theta$ by utilizing \citeauthor{Brenier1987}'s theorem and parameterizing $T_\theta = \nabla \text{PICNN}_\theta$ taking as input the measure at the previous time point $\mu_{i}$ as well as the time interval $\delta t$ during which the cells are expected to evolve (see \S~\ref{sec:neural_primal} for more details).
...
We seek to learn a global, parameterized transport map $\mathcal{T}_\theta$ sensitive to that dosage.% and accurately describing perturbed cellular states at different dosage levels.
...
\subsection{Population Dynamics Conditioned on \emph{Covariates}} \label{sec:eval_covariate}
...
\subsection{Population Dynamics Conditioned on \emph{Actions}}

\looseness -1 To recommend personalized medical procedures for patients, or to improve our understanding of genetic circuits, it is key to be able to predict the outcomes of novel perturbations, arising from combinations of drugs or of genetic perturbations. 
