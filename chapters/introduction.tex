...
Introducing and developing (\colcube{darkblue}) \textbf{inductive biases in deep learning architectures for population dynamics}  is quintessential toward the success and ability of such methods to capture complex dynamics from \emph{unaligned} snapshot data.
...
A framework that facilitates the design of such deep learning architectures that (\colstar{pink}) operate on distributions, i.e., allows modeling heterogeneous populations of particles, (\coldiamond{lightblue}) trained based on samples of \emph{unaligned} distributions, is the \textbf{theory of optimal transport}.

The mathematical foundation of this work further builds on the intuition that perturbations incrementally alter the molecular profiles of cells. This underlying assumption aligns with the theory of \acrfull{OT} that studies the evolution of measures under the \emph{minimum effort} principle. Thus, (\colcube{darkblue}) OT serves naturally as an inductive bias for the deep learning architectures introduced in this thesis.
...
% The modeling of single-cell dynamics has become an essential research area, with implications for understanding biological processes on a fundamental level. Different computational methods have been devised to model these dynamics, and this section reviews some of the prominent approaches.

% Stochastic modeling has been employed to capture the intrinsic noise present in single-cell dynamics. Gillespie's algorithm (Gillespie, 1977) has been foundational in this regard, providing a way to simulate random reactions at the single-cell level. Many variants and improvements on this method have been developed to model complex biochemical networks within single cells (Gibson and Bruck, 2000; Cao et al., 2006).

% ODE-based models have been widely used for single-cell analysis, especially for deterministic modeling where fluctuations can be ignored. Such models have provided insights into cell growth, gene regulation, and signaling pathways (Tyson et al., 2003; Alon, 2007). However, their inability to capture stochastic effects limits their applicability in some scenarios.
...
% Agent-based models (ABMs) provide a more flexible approach to modeling single-cell dynamics by allowing individual entities to interact in complex ways (Walker and Southgate, 2009). These models have proven useful for simulating cell differentiation, migration, and interactions with the environment.

% Recent years have witnessed the application of machine learning techniques for single-cell analysis. Deep learning methods have been utilized to extract complex patterns and perform classification tasks related to single-cell RNA sequencing (scRNA-seq) data (Xu et al., 2020). Other machine learning models, such as random forests and support vector machines, have also been applied to predict single-cell behaviors (Angermueller et al., 2016).

% Hybrid models that combine two or more of the above approaches have also been developed. These models aim to leverage the strengths of different methods to overcome individual limitations. For instance, a combination of stochastic simulations with deterministic ODEs has been proposed to provide a more comprehensive understanding of single-cell processes (Salis and Kaznessis, 2005).

% The computational modeling of single-cell dynamics has evolved considerably, and a wide range of techniques has been proposed to study different aspects of single-cell biology. From deterministic methods to stochastic simulations, agent-based models to machine learning techniques, the field is characterized by a rich diversity of approaches. As technological advancements continue to offer more detailed single-cell data, it is expected that these computational models will further evolve, fostering our understanding of complex biological systems on a cellular level.
...
% Angermueller, C., Pärnamaa, T., Parts, L., Stegle, O., 2016. "Deep learning for computational biology." Molecular Systems Biology, 12(7), pp.878.
...
% Population dynamics have been previously modeled in the context of cell cycle transitions \citep{kafri2013dynamics, kuritz2017relationship} and in the context of scRNA-seq under steady state assumptions \citep{weinreb2018fundamental}.
...
Contrary to contributions of this thesis, however, almost all previous approaches use \acrshort{OT} in the \emph{non-parametric} setting and thus do not allow predictions on unseen samples or extrapolations beyond the measured time horizon.
