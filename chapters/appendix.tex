...
$10,995$ cells were imaged in the DMSO-treated control state and the rest are treated with one of $34$ cancer therapies. Between $2,000$ and $3,000$ cells are profiled per treatment.
...
\paragraph{Feature selection.}
Similar as in \cref{app:dataset_srivatsan}, for evaluation we select the top $50$ marker genes, i.e., those genes most strongly affected by the particular genetic perturbation.
...
\cref{sec:jkonet_cell} and \ref{sec:gsbflow_cell} thereby contain an analysis on the captured lineage branch heterogeneity of the predictions by computing the lineage branch class of each cell using the $k$-NN classifier.
...
The resulting dataset consists of three snapshots taken on day 2, 4, and 6, respectively. While at day 2 most cells are undifferentiated, at later time points cells have developed into neutrophils, monocytes, megakaryocytes (meg), mast cells, lymphoid precursors, erythrocytes, basophils, eosinophils, etc.
