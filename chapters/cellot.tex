...
For this, each input convex neural networks \citep{amos2017input} is trained with loss functions \eqref{eq:makkuva_f_loss}-\eqref{eq:makkuva_f_loss}. We recover the optimal map $T_k$ using the gradient of a convex function $\varphi_k$, i.e., $\nabla \varphi_k$.
...
We evaluate \textsc{CellOT} by analyzing its performance out-of-sample, i.e., on unseen cells originating from the same distribution, as well as on different sample compositions (o.o.d.~setting). 
