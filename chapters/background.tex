...
% , transforming cells from a pluripotent states (capable of becoming any cell type) to a highly specialized ones. 
...
% Gradient flows, or steepest descent curves, are a very classical topic in evolution equations: take a functional F defined on a vector space X, and, instead of looking at points x minizing F (which is related to the static equation \nablaF(x) = 0), we look, given an initial point x0, for a curve starting at x0 and trying to minimize F as fast as possible (in this case, we will solve equations of the form x'(t) = -\nablaF(x(t))). As we speak of gradients (which are element of X, and not of X' as the differential of F should be), it is natural to impose that X is an Hilbert space (so as to identify it with its dual and produce a gradient vector).
