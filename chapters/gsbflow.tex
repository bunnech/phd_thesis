...
In this thesis, we consider the likelihood training framework by \citet{chen2021likelihood} grounded on forward-backward \acrshort{SDE} theory \citep{}. Crucially, these SDEs can be used to construct the likelihood objectives for \acrshortpl{SB} that, surprisingly, generalize the ones for \acrshort{SGM} as special cases. The negative likelihood function for $\paramf$ and $\paramb$ thereby  are given by
\begin{subequations}
\begin{align}
...
Unfortunately, such frameworks necessitates a forward-backward learning process known as the \acrfull{IPF} procedure \citep{fortet1940resolution, kullback1968probability}.
