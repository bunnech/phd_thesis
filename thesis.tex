\RequirePackage{fix-cm}
\documentclass[
    twoside, openright, titlepage, numbers=noenddot,
    cleardoublepage=empty, dottedtoc,
    headings=optiontoheadandtoc,
    abstract=false,
    BCOR=5.5mm, paper=a5, fontsize=10pt, % A5 soft cover
    % BCOR=5.5mm, paper=17cm:24cm, fontsize=10pt, % 17 cm x 24 cm
    % BCOR=5mm, paper=15.59cm:23.39cm, fontsize=10pt, % Royal soft cover
    % BCOR=0mm, paper=15.24cm:22.86cm, fontsize=10pt, % US-Trade hard cover
]{scrreprt}
\usepackage{etex}
\reserveinserts{10}

\input{preamble/general}
\usepackage[final]{pdfpages}

% Bibliography
\usepackage[round, sectionbib]{natbib}
\usepackage[resetlabels]{multibib}
\newcites{own}{ }

% Math definitions
\input{preamble/math}
\input{preamble/acronyms}

\begin{document}

% TODO: Integrate Appendix on datasets into empirical evaluation sections.
% TODO: Integrate Appendix on evaluation metrics into empirical evaluation sections.

\frenchspacing
\raggedbottom%
\selectlanguage{english}
\pagenumbering{roman}
\pagestyle{scrplain}

% Cover
\includepdf[pages={1,{}}]{cover/crop/cover_tmp.pdf}
\cleardoublepage\setcounter{page}{1}

% Frontmatter
\input{frontbackmatter/dirtytitlepage}
\input{frontbackmatter/titlepage}
\input{frontbackmatter/titleback}
\cleardoublepage\input{frontbackmatter/dedication}
\cleardoublepage\input{frontbackmatter/abstract}
\cleardoublepage\input{frontbackmatter/acknowledgments}
\pagestyle{scrheadings}
\cleardoublepage\null

% Table of Contents
\refstepcounter{dummy}
\pdfbookmark[1]{\contentsname}{tableofcontents}
\setcounter{tocdepth}{2} % <-- 2 includes up to subsections in the ToC
\setcounter{secnumdepth}{3} % <-- 3 numbers up to subsubsections
\manualmark%
\markboth{\spacedlowsmallcaps{\contentsname}}{\spacedlowsmallcaps{\contentsname}}
\tableofcontents
\automark[section]{chapter}
\renewcommand{\chaptermark}[1]{\markboth{\spacedlowsmallcaps{#1}}{\spacedlowsmallcaps{#1}}}
\renewcommand{\sectionmark}[1]{\markright{\thesection\enspace\spacedlowsmallcaps{#1}}}

% Notation and Acronyms
\input{preamble/notation}
\clearpage
\cleardoublepage\null

% Mainmatter
\cleardoublepage%
\pagenumbering{arabic}%
\input{chapters/introduction}

\cleardoublepage%
\input{chapters/background}

\part{Static Neural Optimal Transport} \label{part:static_not}
\cleardoublepage%
\input{chapters/cellot}
\input{chapters/condot}

\part{Dynamic Neural Optimal Transport} \label{part:dynamic_not}
\cleardoublepage%
\input{chapters/jkonet}
\input{chapters/gsbflow}
\input{chapters/sbalign}

\cleardoublepage%
\input{chapters/conclusion}

\newpage
\bibliographystyle{plainnat}
\bibliography{references.bib}

\bookmarksetup{startatroot}
\cleardoublepage\input{frontbackmatter/cv}
\cleardoublepage\input{frontbackmatter/publications}

\appendix
\cleardoublepage%
\input{chapters/appendix}

\end{document}
